...
public class AppController {
    ...
    private void configurarCallbacks() {
        ...
        serialService.addDadosListener(dados ->
            SwingUtilities.invokeLater(() -> view.atualizarDados(dados))
        );

        // SerialService → View: mudança de status
        serialService.addStatusListener(msg ->
            SwingUtilities.invokeLater(() -> view.setStatus(formatarStatus(msg)))
        );
    ...
    private void atualizarListaPortas() {
        SwingUtilities.invokeLater(() ->
            view.setPortas(SerialService.listarPortas())
        );
    }
